...
/// Hỗ trợ cả HTTP/HTTPS và SOCKS5 proxies
/// </summary>
public class ProxyHttpClientHandler : HttpClientHandler
{
    ...
    public ProxyHttpClientHandler(ProxyInfo proxyInfo)
    {
        ...
        // SOCKS5 không được hỗ trợ bởi HttpClientHandler
        // Phải sử dụng Socks5HttpHandlerFactory.CreateSocks5Handler() thay thế
        if (_proxyInfo.Type == ProxyType.SOCKS5)
        {
            throw new NotSupportedException(
                $"SOCKS5 proxy is not supported by HttpClientHandler. " +
                $"Use Socks5HttpHandlerFactory.CreateSocks5Handler() instead for proxy {_proxyInfo.Host}:{_proxyInfo.Port}");
        }
